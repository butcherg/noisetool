...
/////////////////////////////////////////////////////////////////////////////
// NoiseMapBuilderBrick class

NoiseMapBuilderBrick::NoiseMapBuilderBrick ():
  m_lowerXBound  (0.0),
  m_lowerZBound  (0.0),
  m_upperXBound  (0.0),
  m_upperZBound  (0.0)
...
void NoiseMapBuilderBrick::Build ()
{
  if ( m_upperXBound <= m_lowerXBound
    || m_upperZBound <= m_lowerZBound
    || m_destWidth <= 0
...
void NoiseMapBuilderBrick::ApplyPositiveBias ()
{
	float min = std::numeric_limits<float>::max(), max = std::numeric_limits<float>::min();
	
	float w = m_pDestNoiseMap->GetWidth();
	float h = m_pDestNoiseMap->GetHeight();
	
	for (unsigned y=0; y<h; y++) {
		for (unsigned x=0; x<w; x++) {
			float v = m_pDestNoiseMap->GetValue(x,y);
			if (min > v) min = v;
			if (max < v) max = v;
		}
	}
	...
	for (unsigned y=0; y<m_pDestNoiseMap->GetHeight(); y++) {
		for (unsigned x=0; x<m_pDestNoiseMap->GetWidth(); x++) {
			m_pDestNoiseMap->SetValue(x, y, m_pDestNoiseMap->GetValue(x,y)-min);
		}
	}
	
}

void NoiseMapBuilderBrick::ApplyBorderGradient (int depth, float elevation)
{
	...
	//2. use min to raise the noisemap positive
	for (unsigned y=0; y<m_pDestNoiseMap->GetHeight(); y++) {
		for (unsigned x=0; x<m_pDestNoiseMap->GetWidth(); x++) {
			m_pDestNoiseMap->SetValue(x, y, m_pDestNoiseMap->GetValue(x,y)-min);
		}
	...
	*/
	...
	//3. calculate the gradient multipliers
	...
	for (unsigned i=0; i<depth; i++) {
		for (unsigned x=l; x<=r; x++) m_pDestNoiseMap->SetValue(x,t,m_pDestNoiseMap->GetValue(x,t) * m[i]);  
		for (unsigned x=l; x<=r; x++) m_pDestNoiseMap->SetValue(x,b,m_pDestNoiseMap->GetValue(x,b) * m[i]); 
		for (unsigned y=t+1; y<=b-1; y++) m_pDestNoiseMap->SetValue(l,y,m_pDestNoiseMap->GetValue(l,y) * m[i]); 
		for (unsigned y=t+1; y<=b-1; y++) m_pDestNoiseMap->SetValue(r,y,m_pDestNoiseMap->GetValue(r,y) * m[i]); 
		l++; r--; t++; b--; 
	}
...
/////////////////////////////////////////////////////////////////////////////
// WriterOpenSCADSurface class
/*
 void WriterOpenSCADSurface::WriteDestFile ()
 {
	if (m_pSourceImage == NULL) {
		throw noise::ExceptionInvalidParam ();
	}
  ...
  // Open the destination file.
  os.open (m_destFilename.c_str (), std::ios::out);
  if (os.fail () || os.bad ()) {
    os.clear ();
    throw noise::ExceptionUnknown ();
  }
  ...
  for (unsigned y=0; y<height; y++) {
	  for (unsigned x=0; x<width; x++) {
		  utils::Color c = m_pSourceImage->GetValue(x,y);
		  float h = (float) (c.red+c.green+c.blue)/3 / 255.0;
		  os << h << " ";
	  }
	  os << std::endl;
  }
 ...
 */
 ...
 WriterOpenSCADSurface::WriterOpenSCADSurface ()
 {
 }
 ...
 void WriterOpenSCADSurface::WriteDestFile ()
 {
  ...
  for (unsigned y=0; y<height; y++) {
	  for (unsigned x=0; x<width; x++) {
		os <<  m_pSourceNoiseMap->GetValue(x,y) << " ";
	  }
	  os << std::endl;
  }
