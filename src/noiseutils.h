...
namespace noise
{
  ...
  namespace utils
  {
    ...
    class NoiseMapBuilderPlane: public NoiseMapBuilder
    {
	 ...
	 /// Builds a planar noise map with brick features.
    ///
    /// This class builds a noise map by filling it with coherent-noise values
    /// generated from the surface of a plane.  The noise map can also be
	/// told to taper the noise values to zero at the edges.
    ///
    /// This class describes these input values using (x, z) coordinates.
    /// Their y coordinates are always 0.0.
    ///
    ...
    /// bounds of the noise map, in units.
    ///
    class NoiseMapBuilderBrick: public NoiseMapBuilder
    {

      public:

        /// Constructor.
        NoiseMapBuilderBrick ();
		...
		/// Applies a gradient to zero to the outer edges of the noisemap.
        ///
        /// @param depth The number of border 'rings' to which to apply
        /// the gradient.  Default: 4.
		/// @param elevation The gradiant that the interpolation starts
        /// with, applied in ring #2.  Ring #1 contains 0.0, the bottom
		/// of the gradiant taper.  Default: 0.4.
		void ApplyBorderGradient (int depth=4, float elevation=0.4);
		
		/// Raises the noisemap so all values are positive
		///
		void ApplyPositiveBias ();
	...
	/// OpenSCAD surface texture writer class.
    ///
    /// This class creates a file OpenSCAD surface format given the
    /// contents of an image object.
    ///
    /// <b>Writing the surface file</b>
    ///
    /// To write the surface array to a file, perform the following steps:
    /// - Pass the filename to the SetDestFilename() method.
    /// - Pass an Image object to the SetSourceImage() method.
    /// - Call the WriteDestFile() method.
    ///
    /// The SetDestFilename() and SetSourceImage() methods must be called
    /// before calling the WriteDestFile() method.
    class WriterOpenSCADSurface
    {

      public:

        /// Constructor.
        WriterOpenSCADSurface ();
        //  m_pSourceImage (NULL)
        //{
        //}
